...
export class VideoRTC extends HTMLElement {
    ...
    oninit() {
        ...
        this.video.addEventListener('error', ev => {
            ...
            // v0.3.3 patch: WebRTC 모드에서 video.src 는 비어있는 상태로 시작
            // (srcObject 만 설정됨). 이때 브라우저가 MEDIA_ERR_SRC_NOT_SUPPORTED 를
            // 발생시키며 연쇄적으로 ws 를 close → 재연결 무한 루프 발생.
            // 빈 src 에서의 MEDIA_ERR_SRC_NOT_SUPPORTED 는 무시하고 계속 진행.
            if (err && err.code === 4 && (!this.video.src || this.video.src === '')) {
                return;
            }
    ...
    onwebrtc() {
        // v0.5.2: Chrome 의 RTCPeerConnection 갯수 제한 (~500) 무한 누적 방지.
        // 이전 PeerConnection 살아있으면 재사용/거부.
        if (this.pc && this.pc.connectionState !== 'closed' && this.pc.connectionState !== 'failed') {
            console.warn('[VideoRTC] 이미 PeerConnection 활성 — 새 PC 생성 skip');
            return;
        }
        let pc;
        try {
            ...
            console.error('[VideoRTC] RTCPeerConnection 생성 실패 (Chrome 제한 도달?):', e);
            // WebSocket 닫고 재시도 멈춤 — 사용자가 새로고침해야 회복
