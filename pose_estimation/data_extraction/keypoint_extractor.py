...
파일: keypoint_extractor.py
설명: 포즈 키포인트 데이터 추출기
      - YOLOv8-Pose 모델 농구 특화 재학습용 데이터셋 자동 추출
      - 3단계 품질 필터 (프레임/인물/프레임집계)
      - 풀프레임 이미지 + YOLO Keypoint .txt 라벨 + JSON 상세 어노테이션
      - 스켈레톤 완전성 필터 (최소 가시 키포인트 + 필수 키포인트)
...
데이터 흐름:
    ...
    PoseEstimator.estimate() → list[NDArray(17,3)] →
    KeypointExtractor.extract_from_frame(detections, frame, keypoints_per_person) →
    3단계 필터 → 프레임 리사이즈 + 키포인트 스케일링 →
    pHash 중복 확인 → JPEG 인코딩 →
    YOLO Keypoint 라벨 생성 + JSON 어노테이션 → 버퍼 축적 →
...
# =============================================================================
# YAML 설정 키
CONFIG_KEY_KEYPOINT_EXTRACTION = "data_extraction.keypoint_extraction"

# 기본값 (YAML 미로드 시 폴백)
DEFAULT_MIN_DETECTIONS: int = 1
...
DEFAULT_MIN_QUALIFYING_PERSONS: int = 1
...
DEFAULT_MAX_TOTAL_SAMPLES: int = 5000
DEFAULT_LOCAL_BASE_PATH: str = "extracted_data/pose/keypoints"
DEFAULT_S3_PREFIX: str = "pose/keypoints"
DEFAULT_S3_BUCKET: str = "courtview-learning"
DEFAULT_SIMILARITY_THRESHOLD: int = 5
DEFAULT_MAX_CACHE_SIZE: int = 2000
...
class KeypointExtractionConfig:
    """
    포즈 키포인트 데이터 추출 설정 (Immutable).

    YAML configs/pose/data_extraction.yaml의 keypoint_extraction 섹션에서 로드.
    """
    ...
    min_bbox_pixels: int = DEFAULT_MIN_BBOX_PIXELS
    ...
    min_qualifying_persons: int = DEFAULT_MIN_QUALIFYING_PERSONS
    ...
    def from_config(
        ...
        Returns:
            KeypointExtractionConfig 인스턴스
        """
        if config_loader is None:
            return cls()

        prefix = CONFIG_KEY_KEYPOINT_EXTRACTION

        enabled = config_loader.get(f"{prefix}.enabled", True)

        # 품질 필터
        qf = f"{prefix}.quality_filter"
        min_det = config_loader.get(f"{qf}.min_detections", DEFAULT_MIN_DETECTIONS)
        min_det_conf = config_loader.get(
            f"{qf}.min_detection_confidence", DEFAULT_MIN_DETECTION_CONFIDENCE
        )
        min_px = config_loader.get(f"{qf}.min_bbox_pixels", DEFAULT_MIN_BBOX_PIXELS)
        kp_conf = config_loader.get(
            f"{qf}.keypoint_confidence_threshold", DEFAULT_KEYPOINT_CONFIDENCE_THRESHOLD
        )
        ...
        critical_raw = config_loader.get(
            f"{qf}.critical_keypoint_indices", DEFAULT_CRITICAL_KEYPOINT_INDICES
        )
        critical_indices = tuple(int(x) for x in critical_raw)
        min_qual = config_loader.get(
            f"{qf}.min_qualifying_persons", DEFAULT_MIN_QUALIFYING_PERSONS
        )
        ...
        max_samples = config_loader.get(
            f"{bat}.max_total_samples", DEFAULT_MAX_TOTAL_SAMPLES
        )

        # 저장
        sto = f"{prefix}.storage"
        base_path = config_loader.get(f"{sto}.local_base_path", DEFAULT_LOCAL_BASE_PATH)
...
class KeypointExtractionSample:
    """
    단일 추출 샘플.

    3단계 필터를 통과한 프레임에서 생성된 학습 데이터.
    풀프레임 이미지(JPEG) + YOLO Keypoint 라벨(.txt) + JSON 어노테이션.
    """
    ...
    # YOLO Keypoint format 라벨 문자열 (한 줄 = 한 사람)
    yolo_label: str = ""
...
@dataclass(slots=True)
class KeypointExtractionStats:
    """
    추출 통계.

    3단계 필터별 통과/거부 카운터 및 전체 통계.
    프레임 레벨 거부 + 개별 인물 레벨 거부를 별도 추적.
    """
    ...
    # 품질 등급 분포
    quality_excellent: int = 0    # >= 15/17
    quality_good: int = 0         # >= 12/17
    quality_fair: int = 0         # >= 8/17

    # 추출 성공
    total_extracted: int = 0       # 추출된 프레임 수
    total_persons_extracted: int = 0  # 추출된 인물 수
    total_flushed: int = 0
...
@dataclass(slots=True)
class KeypointExtractionMetadata:
    """
    추출 세션 메타데이터.
...
class KeypointExtractor:
    ...
    YOLOv8-Pose 모델 농구 특화 재학습용 데이터셋을 자동 추출합니다.
    분석 중 고품질 포즈 키포인트를 3단계 품질 필터로 선별하고,
    풀프레임 이미지 + YOLO Keypoint 라벨 + JSON 어노테이션을 로컬에 저장합니다.
    ...
    3단계 품질 필터:
        ...
        Stage 2 (인물): role, 신뢰도, bbox 크기, 가시 키포인트, 필수 키포인트, 완전성
    ...
    def __init__(
        self,
        config: KeypointExtractionConfig | None = None,
        ...
        """
        self._config: KeypointExtractionConfig | None = config
        self._metrics_collector = metrics_collector
        self._initialized: bool = False

        # 상태
        self._stats: KeypointExtractionStats = KeypointExtractionStats()
        self._buffer: list[KeypointExtractionSample] = []
    ...
    def initialize(
        ...
        with self._lock:
            ...
            if config is not None:
                ...
                self._config = KeypointExtractionConfig.from_config(config_loader)
            ...
            # 상태 초기화
            self._stats = KeypointExtractionStats()
            self._buffer.clear()
    ...
    @property
    def stats(self) -> KeypointExtractionStats:
        """현재 추출 통계."""
        return self._stats
    ...
    def extract_from_frame(
        ...
        """
        프레임의 포즈 키포인트 데이터 추출 시도.

        3단계 품질 필터 → 프레임 리사이즈 + 키포인트 스케일링 →
        pHash 중복 확인 → JPEG 인코딩 → YOLO Keypoint 라벨 생성 →
        JSON 어노테이션 → 버퍼 추가 → 자동 flush.

        Args:
            detections: 프레임 내 모든 선수 감지 결과
            frame: 원본 프레임 (BGR, H×W×3)
            keypoints_per_person: 각 detection의 COCO 17-point 키포인트
                각 원소: (17, 3) NDArray [x_pixel, y_pixel, confidence]
            frame_index: 프레임 번호
        ...
        if not self._initialized or self._config is None:
            raise PoseEstimationException(
                message="KeypointExtractor가 초기화되지 않았습니다",
        ...
        with self._lock:
            ...
            # Stage 2: 개별 인물 필터 → qualifying 인덱스 + 품질 등급
            ...
            annotation = self._generate_json_annotation(
                ...
                qualifying_indices, qualifying_grades,
            ...
            if self._metrics_collector is not None:
                try:
                    counter = self._metrics_collector.counter(
                        "keypoint_extraction_total",
                        help="총 추출된 키포인트 프레임 수",
                    )
                    counter.increment()
                except Exception:
                    pass
    ...
    def _check_frame_validity(
        ...
        Args:
            ...
            frame: 원본 프레임
            keypoints_per_person: 키포인트 배열 리스트
    ...
    def _filter_person_quality(
        ...
        Stage 2c: bbox 크기
        Stage 2d: 가시 키포인트 수
        Stage 2e: 필수 키포인트 (양 어깨 + 양 엉덩이)
        Stage 2f: 스켈레톤 완전성

        Args:
            detection: 단일 감지 결과
            keypoints: COCO 17-point 키포인트 (17, 3)
            frame_h: 프레임 높이
            frame_w: 프레임 너비
    ...
    def _generate_yolo_keypoint_label(
        ...
        """
        YOLO Keypoint format 라벨 문자열 생성.

        형식: ``class cx cy w h kp1_x kp1_y kp1_v ... kp17_x kp17_y kp17_v``
        모든 좌표 정규화 (0~1).

        Args:
            detection: 감지 결과
            keypoints: 스케일링된 (17, 3) 키포인트 (리사이즈 프레임 좌표)
        ...
        for pos, q_idx in enumerate(qualifying_indices):
            ...
            for ki in range(NUM_KEYPOINTS_COCO):
                if ki < len(kps):
                    ...
                    kp_data.append({
                        "index": ki,
                        "name": _COCO_KEYPOINT_NAMES[ki] if ki < len(_COCO_KEYPOINT_NAMES) else f"kp_{ki}",
    ...
    def _scale_keypoints(
        ...
        Args:
            keypoints: 원본 (17, 3) 키포인트 [x, y, conf]
            scale_x: X 방향 스케일
            scale_y: Y 방향 스케일
    ...
    # 메타데이터 생성
    # =========================================================================
    def _create_metadata(self, source_game_id: str) -> KeypointExtractionMetadata:
        """
        추출 세션 메타데이터 생성.
        ...
        return KeypointExtractionMetadata(
